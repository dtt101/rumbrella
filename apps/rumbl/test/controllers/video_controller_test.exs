defmodule Rumbl.VideoControllerTest do
  ...
  @valid_attrs %{url: "http://www.example.com/video", title: "Test", description: "Test video"}
  ...
  @tag login_as: "max"
  test "creates users video and redirects", %{conn: conn, user: user} do
  ...
  test "does not create video and renders errors when invalid", %{conn: conn} do
    ...
    assert video_count(Video) == count
  end

  @tag login_as: "max"
  test "updates user video and redirects to watch", %{conn: conn, user: user} do
    user_video  = insert_video(user, title: "funny cats")
    ...
    assert updated_video.title == "Test"
  end
